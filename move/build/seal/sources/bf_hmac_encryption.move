...
public struct DecryptionResult has copy, drop, store {
    decrypted_blob: Option<vector<u8>>,
    status: String
}
...
public fun decrypt(
    ...
    key_servers_id: &vector<ID>,
    key_servers_pk: &vector<vector<u8>>,
    derived_keys: &vector<vector<u8>>,
): Option<vector<u8>> {
    let mut public_keys: vector<PublicKey> = vector::empty();
    let mut pk_count = 0;
    while (pk_count < key_servers_id.length()) {
        let pk = new_public_key(key_servers_id[pk_count], key_servers_pk[pk_count]);
        public_keys.push_back(pk);
        pk_count = pk_count + 1;
    };

    let mut verified_g1_keys: vector<Element<G1>> = vector::empty();
    let mut dk_count = 0;
    while (dk_count < derived_keys.length()) {
        let dk = derived_keys[dk_count];
        let dk1 = g1_from_bytes(
            &dk,
        );
        verified_g1_keys.push_back(dk1);
        dk_count = dk_count + 1;
    };
    ...
    if (!verify_nonce(&randomness, &encrypted_object.nonce)) {
        event::emit(DecryptionResult {
            decrypted_blob: none(),
            status: b"nonce_verification_failed".to_string(),
        });
        return none()
    };
    ...
    if (
        ...
        event::emit(DecryptionResult {
            decrypted_blob: none(),
            status: b"share_consistency_verification_failed".to_string(),
        });
        return none()
    };

    // Decrypt the blob.
    let result =hmac256ctr::decrypt(
        blob,
        mac,
    ...
    event::emit(DecryptionResult {
        decrypted_blob: result,
        status: b"success".to_string(),
    });
    result
}
